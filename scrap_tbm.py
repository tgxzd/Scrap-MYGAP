...
def extract_mygap_tbm_data(save_to_file=True):
    ...
    # 1. Get the page
    response = requests.get('https://carianmygapmyorganic.doa.gov.my/mygap_tbm_list.php?pagesize=-1')
    ...
    print(f"\nExtracted {len(extracted_data)} valid records with certification numbers")
...
def save_data(data, format='both'):
    ...
    if format in ['json', 'both']:
        json_filename = f"mygap_data_tbm_{timestamp}.json"
...
if __name__ == "__main__":
    # Extract the data
    mygap_data = extract_mygap_tbm_data()
